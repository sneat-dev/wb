...
// TestDiskYAMLFormatEncodesTheReport drives the --format yaml branch,
// which no existing disk test exercises (only text and json are covered):
// it must succeed and produce YAML-shaped output.
func TestDiskYAMLFormatEncodesTheReport(t *testing.T) {
	root := t.TempDir()
...
// TestDiskFindsLowHeadroom drives the len(report.Findings) > 0 branch,
...
func TestDiskFindsLowHeadroom(t *testing.T) {
	root := t.TempDir()
	t.Setenv("HOME", t.TempDir())
...
// TestDiskTextRenderWriteFailureIsSurfaced drives the default (text)
// format's fmt.Fprint error-check: a write failure while rendering the
// report must be returned, not swallowed.
func TestDiskTextRenderWriteFailureIsSurfaced(t *testing.T) {
	root := t.TempDir()
	t.Setenv("HOME", t.TempDir())
	testenv.Isolate(t)

...
// TestCgxc3MigrateCommandRequiresTwoSourceRootsWithoutHierarchical covers
...
func TestCgxc3MigrateCommandRequiresTwoSourceRootsWithoutHierarchical(t *testing.T) {
	t.Parallel()
...
// TestCgxc3MigrateCommandRejectsHierarchicalOnlyFlagsWithoutHierarchical
// covers the guard that refuses --commit (and its hierarchical-only
// siblings) unless --hierarchical is also set.
func TestCgxc3MigrateCommandRejectsHierarchicalOnlyFlagsWithoutHierarchical(t *testing.T) {
	t.Parallel()
	cmd := newMigrateCmd(&invocation{})
	var out strings.Builder
	cmd.SetOut(&out)

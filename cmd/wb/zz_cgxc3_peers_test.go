...
// TestCgxc3RefuseExistingTokenFileReportsNonNotExistError covers the
...
// never a silent nil.
func TestCgxc3RefuseExistingTokenFileReportsNonNotExistError(t *testing.T) {
	t.Parallel()
	dir := t.TempDir()
...
// TestCgxc3LoadPeerUpstreamStateReportsUnreadableFile covers the
...
func TestCgxc3LoadPeerUpstreamStateReportsUnreadableFile(t *testing.T) {
	t.Parallel()
	dir := t.TempDir()
	blocker := filepath.Join(dir, "blocker")
	if err := os.WriteFile(blocker, []byte("x"), 0o600); err != nil {
...
// TestCgxc3WritePeerDetailRendersConnectedSession covers writePeerDetail's
// (cmd/wb/peers.go) detail.Session != nil branch: a non-nil Session must
// render "connected" instead of the zero-value "none".
func TestCgxc3WritePeerDetailRendersConnectedSession(t *testing.T) {
	t.Parallel()

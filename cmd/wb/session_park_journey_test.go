...
func TestSessionParkResumeAcrossProcessTransport(t *testing.T) {
	...
	if parked.MemberCount != len(parkedState.Bundle.Worktrees) {
		t.Fatalf("park reported %d members, aggregate holds %d", parked.MemberCount, len(parkedState.Bundle.Worktrees))
	}
	...
	if err := os.WriteFile(sourceConfig, []byte("session_move:\n  targets:\n    target:\n      default_courier: ssh\n      ssh:\n        host: journey-target\n"), 0o600); err != nil {
		t.Fatal(err)
	}
	...
	// The public resume output no longer republishes the successor record or
	// the full receipt; it reports identity and a receipt digest, and the
	// durable aggregate remains the authority for the rest.
	var resumed struct {
		Status               string             `json:"status"`
		SuccessorWBSessionID string             `json:"successor_wb_session_id"`
		MemberCount          int                `json:"member_count"`
		ReceiptDigest        sessionmove.Digest `json:"receipt_digest"`
	}
	if err := json.Unmarshal(resumedRaw, &resumed); err != nil {
		t.Fatal(err)
	}
	if resumed.Status != string(sessionpark.StatusResumed) || resumed.SuccessorWBSessionID == "" ||
		resumed.ReceiptDigest == "" || resumed.MemberCount != len(sourceMembers) {
		t.Fatalf("remote resume result = %#v, want one durable receipt digest and its exact successor identity", resumed)
	}
	...
	if state.Status != sessionpark.StatusResumed || state.Successor == nil ||
		state.Successor.WBSessionID != resumed.SuccessorWBSessionID ||
		state.Successor.PredecessorWBSessionID != source.WBSessionID ||
		state.RemoteReceipt == nil || state.RemoteReceipt.SuccessorWBSessionID != resumed.SuccessorWBSessionID ||
		len(state.RemoteReceipt.Members) != len(sourceMembers) || len(state.Events) == 0 {
		t.Fatalf("source terminal custody state = %#v", state)
	}
	// The durable aggregate is the authority for the full receipt and
	// successor record now that the public surface reports only identity.
	receipt := state.RemoteReceipt
	terminalEvent := state.Events[len(state.Events)-1]
	if terminalEvent.Type != "resumed" || terminalEvent.RemoteResumeID != receipt.ResumeID || terminalEvent.TargetMachine != "target" || terminalEvent.RequestDigest != receipt.RequestDigest || terminalEvent.Successor == nil || terminalEvent.Successor.WBSessionID != receipt.SuccessorWBSessionID {
		t.Fatalf("source terminal custody event = %#v", terminalEvent)
	}
	assertJourneyTargetBundle(t, targetHome, receipt, sourceMembers)
	targetSessions, err := session.List(filepath.Join(targetHome, session.DirName))
	if err != nil {
		t.Fatal(err)
	}
	if len(targetSessions) != 1 || targetSessions[0].WBSessionID != receipt.SuccessorWBSessionID || targetSessions[0].State != session.StateLive {
		t.Fatalf("target session registration = %#v", targetSessions)
	}
	targetEvents := make(map[string][]byte, len(receipt.Members))
	...
	resumedAgainRaw := runJourneyWB(t, binary, sourceRoot, "session", "resume", parked.ParkedSessionID,
		"--to", "target", "--via", "ssh", "--config", sourceConfig, "--format", "json")
	// Replay must converge on the byte-identical public result, and the
	// durable aggregate must be untouched (asserted immediately below).
	...
	if resumedAgain.Status != string(sessionpark.StatusResumed) ||
		resumedAgain.SuccessorWBSessionID != resumed.SuccessorWBSessionID ||
		resumedAgain.ReceiptDigest != resumed.ReceiptDigest ||
		resumedAgain.MemberCount != resumed.MemberCount {
...
// runJourneyWB returns stdout only. stderr is deliberately not merged: it
// carries operator prompts and diagnostics, while stdout carries the
// machine-readable record that --format json consumers parse.

...
func TestCgxc1HandleRemoteOperationStatusSuccess(t *testing.T) {
	home := agentTestEnv(t)
...
func TestCgxc1HandleRemoteOperationAwaitSuccessCarriesWaitTimeout(t *testing.T) {
	home := agentTestEnv(t)
	record := seedAgentRun(t, home, func(r *agents.Record) { r.State = agents.StateCompleted })
	inv := &invocation{projectsRoot: os.Getenv("WB_PROJECTS_ROOT")}
...
func TestCgxc1HandleRemoteOperationRefusesOversizedLogFetch(t *testing.T) {
	home := agentTestEnv(t)
...
func TestCgxc1HandleRemoteOperationRejectsUnsupportedOperation(t *testing.T) {
	t.Parallel()
	var response agents.RemoteResponse

...
func newRootCmd() *cobra.Command {
	...
	root := &cobra.Command{
		...
		PersistentPreRunE: func(cmd *cobra.Command, args []string) error {
			if err := rejectIgnoredPersistentFlags(cmd); err != nil {
				return err
			}
			commandStarted = true
			return nil
...
var persistentFlagSupport = map[string]map[string]bool{
	"projects-root": {
		"sync": true, "run": true, "migrate": true, "deps": true, "ci": true,
		"hooks": true, "coverage": true, "verify": true, "check": true,
		"status": true, "worktree": true,
	},
	"filter": {
		"sync": true, "run": true, "deps": true, "ci": true, "hooks": true,
		"coverage": true, "verify": true, "check": true, "status": true,
		"worktree list": true, "worktree cleanup": true, "worktree rename": true,
	},
	"org": {"run": true, "deps": true},
	...
	"non-interactive": {
		"sync": true, "run": true, "migrate": true, "deps": true, "ci": true,
		"hooks": true, "coverage": true, "verify": true, "check": true,
		"status": true, "worktree": true, "version": true, "self-update": true,
	},
}

func rejectIgnoredPersistentFlags(cmd *cobra.Command) error {
	commandID := persistentCommandID(cmd)
	for flag, supported := range persistentFlagSupport {
		candidate := cmd.Flags().Lookup(flag)
		if candidate == nil || !candidate.Changed {
			continue
		}
		if supported[commandID] || supported[firstCommand(commandID)] {
			continue
		}
...
func firstCommand(id string) string {

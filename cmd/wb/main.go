...
func newRootCmd() *cobra.Command {
	...
	root.AddCommand(
		...
		groupedRootCommand(newReviewCmd(), rootGroupChange),
...
var persistentFlagSupport = map[string]map[string]bool{
	"projects-root": {
		...
		"worktree own": true,
		"stream start": true, "stream join": true, "stream status": true, "stream end": true, "stream delete": true, "stream sync": true, "stream absorb": true,
		"review record":    true,

...
// TestCgxc2MergeAgentHookSettingsSurfacesNonNotExistReadErrors proves the
...
func TestCgxc2MergeAgentHookSettingsSurfacesNonNotExistReadErrors(t *testing.T) {
	t.Parallel()
	dir := t.TempDir()
...
// TestCgxc2AgentHookEntryPresentRejectsNonObjectEntry proves the top-level
// `entry.(map[string]any)` !ok branch returns false rather than panicking.
func TestCgxc2AgentHookEntryPresentRejectsNonObjectEntry(t *testing.T) {
	t.Parallel()
...
// TestCgxc2AgentHookEntryPresentSkipsNonObjectHandlers proves a non-object
// handler element is skipped (continue), while a sibling matching handler
// still yields true.
func TestCgxc2AgentHookEntryPresentSkipsNonObjectHandlers(t *testing.T) {
	t.Parallel()
...
// TestCgxc2AgentHookEntryMatcherStaleRejectsNonObjectEntry proves the
...
func TestCgxc2AgentHookEntryMatcherStaleRejectsNonObjectEntry(t *testing.T) {
	t.Parallel()

...
// TestRwi01SweepAppliesStrictConfigPromotingReportFindings covers sweep's
...
func TestRwi01SweepAppliesStrictConfigPromotingReportFindings(t *testing.T) {
	t.Parallel()
...
// TestRwi01SweepSortsOutcomesByRepositoryThenDirectory covers sweep's
// sort.Slice directory tiebreak: two modules under the same repository
// slug must land ordered by their module directory.
func TestRwi01SweepSortsOutcomesByRepositoryThenDirectory(t *testing.T) {
	t.Parallel()
	rootA := t.TempDir()
	rootB := t.TempDir()
...
// TestRwi01WriteBucketsSortsByCountThenKey covers writeBuckets' sort
// comparator on both sides: a count difference (descending) and, for two
// buckets with an EQUAL count, the key tiebreak.
func TestRwi01WriteBucketsSortsByCountThenKey(t *testing.T) {
	t.Parallel()

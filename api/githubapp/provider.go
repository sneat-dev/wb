...
type ProjectionStore interface {
	...
	ListLatestMerges(context.Context, int) ([]LatestMerge, error)
}
...
func (model StoreReadModel) Dashboard(ctx context.Context, viewer Viewer) (Access[Dashboard], error) {
	...
	return Access[Dashboard]{Visibility: VisibilityPublic, Value: Dashboard{GeneratedAt: updated, Summary: summary}}, nil
}
...
func (model StoreReadModel) Stats(ctx context.Context, viewer Viewer, scope Scope, id string) (Access[Stat], error) {
	...
	return Access[Stat]{Visibility: VisibilityPublic, Value: Stat{Scope: scope, ID: document.ID, DisplayName: document.DisplayName, Summary: document.Summary, UpdatedAt: document.UpdatedAt}}, nil
}
...
func (model StoreReadModel) Series(ctx context.Context, viewer Viewer, scope Scope, id, metric string) (Access[Series], error) {
	...
	return Access[Series]{Visibility: VisibilityPublic, Value: Series{Scope: scope, ID: id, Metric: metric, Points: series.Points}}, nil
}
...
func (model StoreReadModel) Leaderboard(ctx context.Context, viewer Viewer, metric string) (Access[Leaderboard], error) {
	...
	if !viewer.Authenticated || !viewer.Member {
		return Access[Leaderboard]{}, ErrPrivateData
	}
	return Access[Leaderboard]{Visibility: VisibilityPrivate, Value: Leaderboard{Metric: document.Metric, Entries: document.Entries}}, nil
}
...
func (model StoreReadModel) LatestMerges(ctx context.Context, viewer Viewer, limit int) (Access[[]LatestMerge], error) {
	...
	merges, err := model.Store.ListLatestMerges(ctx, limit)
	if err != nil {
		return Access[[]LatestMerge]{}, err
	}
	return Access[[]LatestMerge]{Visibility: VisibilityPublic, Value: merges}, nil
}
...
func (model StoreReadModel) visible(ctx context.Context, viewer Viewer, document ProjectionDocument) (bool, error) {
	...
	return model.Membership.Member(ctx, viewer, document.Scope, document.ID)
}

...
func TestVerifyPublicEligibilityAcceptsOnlyExplicitRootREADMESections(t *testing.T) {
	...
	for name, markdown := range map[string]string{
		...
		t.Run(name, func(t *testing.T) {
			evidence, err := VerifyPublicEligibility("github.com/acme/widgets", "https://github.com/acme/widgets/blob/main/README.md", markdown, verifiedAt)
			if err != nil {
				t.Fatal(err)
			}
			if evidence.Repository != "github.com/acme/widgets" || evidence.READMEURL != "https://github.com/acme/widgets/blob/main/README.md" || !evidence.VerifiedAt.Equal(verifiedAt) {
...
func TestVerifyPublicEligibilityRejectsAmbiguousOrUnrelatedLinks(t *testing.T) {
	for name, markdown := range map[string]string{
		...
		t.Run(name, func(t *testing.T) {
			_, err := VerifyPublicEligibility("github.com/acme/widgets", "https://github.com/acme/widgets/blob/main/README.md", markdown, time.Now())
...
func TestValidatePublicEligibilityRejectsNonCanonicalEvidence(t *testing.T) {
	valid := PublicEligibility{Repository: "github.com/acme/widgets", READMEURL: "https://github.com/acme/widgets/blob/main/README.md", VerifiedAt: time.Now()}
	...
	for name, mutate := range map[string]func(*PublicEligibility){
		...
		"README belongs to a different repository": func(e *PublicEligibility) { e.READMEURL = "https://github.com/acme/other/blob/main/README.md" },
		"README is not root README":                func(e *PublicEligibility) { e.READMEURL = "https://github.com/acme/widgets/blob/main/docs/README.md" },
		"README is not HTTPS":                      func(e *PublicEligibility) { e.READMEURL = "http://github.com/acme/widgets/blob/main/README.md" },
		"README has a query": func(e *PublicEligibility) {
			e.READMEURL = "https://github.com/acme/widgets/blob/main/README.md?plain=1"
		},

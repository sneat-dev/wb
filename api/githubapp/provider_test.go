...
func (store providerStore) ListLatestMerges(context.Context, int) ([]LatestMerge, error) {
...
func TestStoreReadModelAggregatesAndDiscloses(t *testing.T) {
	store := providerStore{documents: []ProjectionDocument{providerDocument(true), {Scope: ScopeRepository, ID: "github.com/acme/private", DisplayName: "private", UpdatedAt: time.Unix(20, 0), Summary: Summary{Repositories: 2}}}, series: SeriesDocument{Points: []SeriesPoint{{Value: 7}}}, board: LeaderboardDocument{Metric: "landed", Entries: []LeaderboardEntry{{Rank: 1}}}, merges: []LatestMerge{{Repository: "acme/widgets"}}}
	...
	if err != nil || dashboard.Value.Summary.Repositories != 3 || dashboard.Value.GeneratedAt != time.Unix(20, 0) {
	...
	if _, err = model.Leaderboard(context.Background(), Viewer{}, "landed"); !errors.Is(err, ErrPrivateData) {
		t.Fatalf("anonymous leaderboard err = %v", err)
	}
}

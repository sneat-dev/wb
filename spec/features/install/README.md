...
installed. The behavior is not specified here: wb binds the shared
...
install methods, and every failure guarantee. This Feature specifies only what
is wb's own — the command wiring and wb's exit-code mapping.
...
| [Self-Update](../self-update/README.md) | Sibling command built on the same fleet catalog (`cliinstall.ByID("wb")`); `wb install wb` is reported as already installed with a `wb self-update` pointer rather than reinstalling. |
...
- Should `wb install` gain the `upgrade` command's own wiring once

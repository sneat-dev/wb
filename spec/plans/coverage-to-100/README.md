...
Route git, gh and other subprocess calls through one runner interface, with a fake that can fail on demand. This is behaviour-preserving and makes about 1,500–2,000 uncovered statements reachable (estimate). The allow-list, found by grepping every `ExtraFiles` assignment repo-wide (2026-09-23) and tracing each to its launcher (which sets `command.ExtraFiles`) and child (which reads fd 3+ via `os.NewFile`):
...
2. **Baseline (task-3b).** Confirm: no committed baseline file — the baseline is the per-package uncovered counts go-ci's coverage job publishes on every push to main, which requires exempting that job from the validation-reuse skip on push events (**a CI-cost choice: a full coverage run on every push to main, including ones that skip today via validation-reuse**), with a wall-time-budgeted merge-base fallback when no artifact exists; the nightly job is not a baseline producer.
...
5. **Should all 56 functions over 150 lines be split regardless of coverage need?** Task-12 splits only the 6 in `internal/orchestrate/worktree_merge.go` and `internal/worktrees/lifecycle.go`, and leaves the other 50 (`_research/long-functions.txt`) to be split package-by-package, only when a wave needs it, each in its own refactor PR. An alternative is splitting all 56 up front as a dedicated readability pass, independent of whether any wave needs it.

...
7. **Baseline (was Open Question 2, task-3b).** "CI artifact, run every push (Recommended)": no hand-edited baseline file; go-ci's coverage job publishes per-package untested counts on every push to main, exempt from the validation-reuse skip. The founder accepted the cost of one full coverage run per merge to main.
8. **#646 and #629 (was Open Question 3, task-3c).** "Agent lanes do it (Recommended)": this programme rebases both as normal lanes (Sonnet implements, Opus reviews, the supervisor lands), rather than waiting for alex.

...
if ! grep -Fq "if: \${{ !cancelled() && github.event_name == 'pull_request' && needs.test.result == 'success' && needs.go-scope.outputs.required == 'true' }}" "$root/.github/workflows/go-ci.yml"; then

...
`branch list --with-prs` opts into GitHub PR history for selected remote branch rows. Its default list path does not query PRs; `branch cleanup` checks open PRs for eligible remote deletion candidates.

...
acknowledge-landed-failed`/`acknowledge-retired-landed`/`seal-validation-failed`/`supersede-validation-failed` expose `--mode` and
...
| `worktree merge`, `merge prepare` (including `--rebatch-receipt`), `merge land`, `merge resume` (including PR-only `--stop-before-merge`), `merge revert`, `merge acknowledge-landed-failed`, `merge acknowledge-retired-landed`, `merge seal-validation-failed`, `merge supersede-validation-failed` | yes | rejected | rejected | yes |

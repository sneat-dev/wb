---
name: wb-deps
description: Inspect dependency topology and set or propagate dependency versions with WB. Use to find consumers, pin an exact GitHub Actions or Go version, plan release order, or update dependent repositories in verified waves.
---
...
Start read-only. `graph` is read-only; `set` and `bump` provide `--dry-run`.

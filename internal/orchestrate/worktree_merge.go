...
func PrepareWorktreeMerge(ctx context.Context, options WorktreeMergePrepareOptions) (WorktreeMergeReceipt, error) {
	...
	receipt := WorktreeMergeReceipt{
		...
		ResumeArgs:      []string{"worktree", "merge", "resume", receiptPath},
...
func LandWorktreeMerge(ctx context.Context, options WorktreeMergeLandOptions) (WorktreeMergeReceipt, error) {
	...
	if decision.Route == WorktreeMergeRouteDirect {
		refspec := receipt.Candidate.SHA + ":refs/heads/" + receipt.Target
		if _, _, err := runCommand(ctx, options.Timeout, options.Retry, receipt.Candidate.Worktree, "git", "push", "origin", refspec); err != nil {
			return failWorktreeMergeReceipt(receipt, WorktreeMergeConflict, fmt.Errorf("direct target push failed without force: %w", err))
		}
	} else {
		refspec := "HEAD:refs/heads/" + receipt.Candidate.Branch
		if _, _, err := runCommand(ctx, options.Timeout, options.Retry, receipt.Candidate.Worktree, "git", "push", "-u", "origin", refspec); err != nil {
...
func worktreeMergePRText(ctx context.Context, receipt WorktreeMergeReceipt) (string, string, error) {
	...
	title := fmt.Sprintf("Merge %d worktree candidates into %s", len(receipt.Sources), receipt.Target)
	if len(subjects) == 1 {
		title = subjects[0]
	}
...
func pullRequestLandingReceipt(ctx context.Context, receipt WorktreeMergeReceipt, options WorktreeMergeLandOptions) (string, bool, error) {
	...
	if view.HeadRefOID != receipt.Candidate.SHA || view.BaseRefName != receipt.Target {
		return "", false, fmt.Errorf("pull-request landing receipt does not match exact head %s and target %s", receipt.Candidate.SHA, receipt.Target)
	}
...
func canRefreshWorktreeMergeReceipt(ctx context.Context, prior WorktreeMergeReceipt, sources []WorktreeMergeSource) (bool, error) {
	switch prior.Status {
	case WorktreeMergePreparing, WorktreeMergePrepared, WorktreeMergeConflict, WorktreeMergeValidationFailed:
	default:
		return false, nil
	}
	if prior.PullRequest != "" || prior.LandingSHA != "" || prior.Candidate.Worktree == "" || prior.Candidate.Branch == "" || len(prior.Sources) != len(sources) {
		return false, nil
	}
	advanced := false

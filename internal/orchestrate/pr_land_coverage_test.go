...
func TestOrchCovDeleteRemoteBranchVerifiesTheEffect(t *testing.T) {
	...
	t.Run("deleted and absent", func(t *testing.T) {
		...
		want := "push --force-with-lease=refs/heads/candidate:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa origin :refs/heads/candidate"
		if strings.TrimSpace(string(args)) != want {
			t.Fatalf("git args = %q, want %q", strings.TrimSpace(string(args)), want)
		}
	})

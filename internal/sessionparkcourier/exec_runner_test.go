...
func TestExecCommandRunnerRunWritesInputAndCapturedOutput(t *testing.T) {
	...
	fake.Expect(func(c runnertest.Call) bool {
		return c.Op == "RunWithInput" && c.Name == "ssh" && string(c.Input) == "envelope"
	...
	if err := execRunner.Run(context.Background(), "ssh", []string{"host"}, []byte("envelope"), &stdout, &stderr); err != nil {
		t.Fatalf("Run: %v", err)
	}

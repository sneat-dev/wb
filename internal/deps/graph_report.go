...
func (graph Graph) Markdown() string {
	...
	output.WriteString("| Dependency | Version | Consumer repository | Consumer module | Manifest | Kind | Provider repository |\n")
	output.WriteString("|---|---|---|---|---|---|---|\n")
	for _, requirement := range graph.Requirements {
		...
		fmt.Fprintf(&output, "| `%s` | `%s` | `%s` | `%s` | `%s` | `%s` | `%s` |\n",
			...
			requirement.ConsumerModule, requirement.Manifest, kind, provider)
	}
...
:root{color-scheme:light dark;font-family:Inter,ui-sans-serif,system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;background:#eef2ff;color:#0f172a}*{box-sizing:border-box}body{margin:0;background:linear-gradient(145deg,#eef2ff 0,#f8fafc 42%,#ecfeff 100%);min-height:100vh}.shell{max-width:1680px;margin:auto;padding:28px}.header{display:flex;justify-content:space-between;gap:24px;align-items:end;margin-bottom:18px}.eyebrow{text-transform:uppercase;letter-spacing:.14em;font-size:12px;font-weight:750;color:#4f46e5}h1{font-size:clamp(28px,4vw,48px);margin:5px 0 4px;letter-spacing:-.04em}.lede{margin:0;color:#475569}.summary{display:grid;grid-template-columns:repeat(6,minmax(110px,1fr));gap:10px;margin:20px 0}.metric{background:rgba(255,255,255,.8);border:1px solid #cbd5e1;border-radius:14px;padding:14px;box-shadow:0 6px 24px rgba(15,23,42,.05)}.metric b{display:block;font-size:24px}.metric span{font-size:12px;color:#64748b}.toolbar{display:flex;flex-wrap:wrap;gap:10px;align-items:center;background:#fff;border:1px solid #cbd5e1;border-radius:14px;padding:10px;margin-bottom:12px}.tabs{display:flex;gap:4px}.tabs button,.tool{border:1px solid #cbd5e1;background:#f8fafc;color:#334155;border-radius:9px;padding:8px 12px;font:inherit;cursor:pointer}.tabs button[aria-selected=true]{background:#1d4ed8;color:#fff;border-color:#1d4ed8}.search{min-width:230px;flex:1;border:1px solid #cbd5e1;border-radius:9px;padding:9px 11px;font:inherit;background:#fff;color:#0f172a}.spacer{flex:1}.graph-frame{background:#f8fafc;border:1px solid #cbd5e1;border-radius:18px;box-shadow:0 18px 60px rgba(15,23,42,.10);overflow:auto;min-height:520px}.graph-panel{display:none;transform-origin:0 0}.graph-panel.active{display:block}.graph-panel svg{display:block;width:100%;min-width:900px;height:auto}.evidence{margin-top:22px;background:rgba(255,255,255,.85);border:1px solid #cbd5e1;border-radius:16px;padding:18px;overflow:auto}table{width:100%;border-collapse:collapse;font-size:13px}th,td{text-align:left;padding:8px;border-bottom:1px solid #e2e8f0;white-space:nowrap}th{color:#475569}.hint{font-size:12px;color:#64748b}.node.dim,.edges>g.dim{opacity:.1}.node.highlight,.edges>g.highlight{opacity:1}.hide-current .node:not(.behind),.hide-current .edges>g:not(:has(.behind)){opacity:.12}@media(max-width:900px){.summary{grid-template-columns:repeat(3,1fr)}}@media(max-width:760px){.shell{padding:16px}.header{display:block}.summary{grid-template-columns:repeat(2,1fr)}.toolbar{align-items:stretch}.tabs{overflow:auto}.graph-frame{min-height:400px}.evidence{padding:12px}}@media(prefers-color-scheme:dark){:root{background:#0f172a;color:#e2e8f0}body{background:linear-gradient(145deg,#111827,#0f172a 55%,#082f49)}.metric,.toolbar,.evidence{background:rgba(15,23,42,.88);border-color:#334155}.lede,.hint,.metric span,th{color:#94a3b8}.search,.tabs button,.tool{background:#1e293b;color:#e2e8f0;border-color:#475569}.graph-frame{border-color:#334155}}
	...
	output.WriteString(`</div><input class="search" type="search" placeholder="Search nodes…" aria-label="Search graph nodes"><label class="hint"><input type="checkbox" class="outdated"> Highlight behind</label><span class="spacer"></span><button class="tool zoom-out" type="button" aria-label="Zoom out">−</button><button class="tool reset" type="button">Reset</button><button class="tool zoom-in" type="button" aria-label="Zoom in">+</button></section><div class="graph-frame">`)
	for _, view := range views {
		class := "graph-panel"
		if view == defaultView {
			class += " active"
	...
	output.WriteString(`</div><p class="hint">Click or focus a node to highlight its complete upstream and downstream path. “Fleet-highest” compares only versions observed in this report.</p><details class="evidence"><summary><b>Canonical requirement evidence</b></summary><table><thead><tr><th>Dependency</th><th>Version</th><th>Consumer</th><th>Module</th><th>Manifest</th><th>Kind</th></tr></thead><tbody>`)
	for _, requirement := range graph.Requirements {
		kind := "direct"
		if requirement.Indirect {
			kind = "indirect"
		}
		fmt.Fprintf(&output, `<tr><td><code>%s</code></td><td><code>%s</code></td><td>%s</td><td><code>%s</code></td><td><code>%s</code></td><td>%s</td></tr>`,
			...
			html.EscapeString(requirement.ConsumerModule), html.EscapeString(requirement.Manifest), kind)
	}
	output.WriteString(`</tbody></table></details></main><script>
(()=>{let scale=1;const panels=[...document.querySelectorAll('.graph-panel')];const active=()=>document.querySelector('.graph-panel.active');function resetHighlight(){active().querySelectorAll('.node,.edges>g').forEach(e=>e.classList.remove('dim','highlight','selected'))}function choose(view){panels.forEach(p=>p.classList.toggle('active',p.dataset.panelView===view));document.querySelectorAll('[data-select-view]').forEach(b=>b.setAttribute('aria-selected',String(b.dataset.selectView===view)));scale=1;active().style.transform='';applySearch()}document.querySelectorAll('[data-select-view]').forEach(b=>b.addEventListener('click',()=>choose(b.dataset.selectView)));function highlight(id){resetHighlight();const panel=active(),nodes=[...panel.querySelectorAll('.node')],edges=[...panel.querySelectorAll('.edges>g')],seen=new Set([id]);let changed=true;while(changed){changed=false;edges.forEach(e=>{const a=e.dataset.from,b=e.dataset.to;if(seen.has(a)||seen.has(b)){if(!seen.has(a)){seen.add(a);changed=true}if(!seen.has(b)){seen.add(b);changed=true}}})}nodes.forEach(n=>n.classList.add(seen.has(n.dataset.nodeId)?'highlight':'dim'));edges.forEach(e=>e.classList.add(seen.has(e.dataset.from)&&seen.has(e.dataset.to)?'highlight':'dim'));const selected=nodes.find(n=>n.dataset.nodeId===id);if(selected)selected.classList.add('selected')}function bind(){document.querySelectorAll('.node').forEach(n=>{n.addEventListener('click',()=>highlight(n.dataset.nodeId));n.addEventListener('keydown',e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();highlight(n.dataset.nodeId)}})})}const search=document.querySelector('.search');function applySearch(){resetHighlight();const query=search.value.trim().toLowerCase();if(!query)return;active().querySelectorAll('.node').forEach(n=>n.classList.toggle('dim',!n.dataset.search.includes(query)))}search.addEventListener('input',applySearch);document.querySelector('.outdated').addEventListener('change',e=>panels.forEach(p=>p.classList.toggle('hide-current',e.target.checked)));document.querySelector('.zoom-in').addEventListener('click',()=>{scale=Math.min(2.5,scale+.2);active().style.transform='scale('+scale+')'});document.querySelector('.zoom-out').addEventListener('click',()=>{scale=Math.max(.5,scale-.2);active().style.transform='scale('+scale+')'});document.querySelector('.reset').addEventListener('click',()=>{scale=1;active().style.transform='';search.value='';document.querySelector('.outdated').checked=false;panels.forEach(p=>p.classList.remove('hide-current'));resetHighlight()});bind()})();
</script></body></html>`)

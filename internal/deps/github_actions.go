...
func rewriteGitHubActions(contents []byte, name string, target Target, apply, allowDowngrade bool) ([]byte, []Decision, error) {
	...
	for index, lineWithEnding := range lines {
		...
		decision := Decision{
			Dependency: target.Dependency, Ecosystem: EcosystemGitHubActions, File: name, Selector: "uses:" + target.Dependency, BeforeRef: beforeRef, BeforeVersion: beforeVersion,

...
func TestDaemonServiceRunsDurableOperationWithBoundedReceipt(t *testing.T) {
	...
	service, err := NewService(root, "test-build", "17")
	if err != nil {
		t.Fatal(err)
	}
	...
	duplicate, err := service.SubmitOperation(context.Background(), connect.NewRequest(&daemonv1.SubmitOperationRequest{
		IdempotencyKey: "test-operation", WorkingDirectory: root, Argv: []string{"different"}, LocalRawCommand: true,
	}))
	if err != nil {
		t.Fatal(err)
	}
...
func TestDaemonServiceDoesNotRunAfterRunningTransitionPersistenceFailure(t *testing.T) {
	...
	service, err := NewService(root, "test-build", "1")
	if err != nil {
		t.Fatal(err)
	}
...
func TestDaemonServiceRejectsUntrustedEnvironmentPersistence(t *testing.T) {
	service, err := NewService(t.TempDir(), "test-build", "1")
	if err != nil {
		t.Fatal(err)
	}
...
func TestDaemonServiceRestartResumesQueuedAndFencesRunningOperations(t *testing.T) {
	root := t.TempDir()
	t.Setenv("WB_DAEMON_TEST_HELPER", "1")
	before, err := NewService(root, "old-build", "3")
	if err != nil {
		t.Fatal(err)
	}
	...
	after, err := NewService(root, "new-build", "4")
	if err != nil {
		t.Fatal(err)
	}

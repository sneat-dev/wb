...
// transport's declared [Capabilities] allow for delivery.Class — a
// transport must downgrade and report [ModeRecordOnly] rather than claim it
// advised or submitted when it did not (REQ:recorded-only-receipt-state's
// principle, applied here to this package's own in-memory Receipt rather
// than to sessionmove.MessageReceipt's wire schema, which Task 4 owns
// separately).
type Receipt struct {
	Operation Operation
...
// mechanism itself does not need to). [Transport.Deliver] implementations
// call this themselves rather than trusting a caller-supplied mode.
//
// [EventOwnPROutcome] can resolve [ModeSubmit] only when both LiveStatus
// and EmptyInputEvidence are true. No shipped [Capabilities] value declares
// EmptyInputEvidence true today, so this branch is unreachable in practice,
// by construction, not by circumstance —
// AC:guards-are-unconditional-and-block-submission. Absence of the evidence
// is itself a guard failure: it is never treated as grounds to advise
// instead, because REQ:prompt-restricted-to-own-pr-outcome reserves the
// own-PR-outcome class for submit-or-record only, never advisory.
//
// [EventSuccessorMessage] can resolve [ModeSubmit] only when LineageSubmit
// is true — never because of LiveStatus, EmptyInputEvidence, or
// AdvisoryDelivery, which govern only the daemon-wake classes. A transport
// that declares LineageSubmit true (tmux) still never reaches ModeSubmit
// for [EventOwnPROutcome] or [EventAdvisory]: LineageSubmit is consulted in
// exactly one branch, this one (AC:tmux-never-delivers-unguarded's
// guarantee holds regardless of LineageSubmit).
//
// [EventAdvisory] resolves [ModeAdvisory] only when AdvisoryDelivery is
...
// resolves ModeRecordOnly for it (AC:tmux-never-delivers-unguarded,

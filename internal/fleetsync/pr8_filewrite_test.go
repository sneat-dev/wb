...
func TestOverwriteRemovalReceiptInjectedHonoursInjectedFailures(t *testing.T) {
	t.Parallel()
	for _, step := range []filewrite.Step{
		...
		t.Run(string(step), func(t *testing.T) {
			...
			if err := overwriteRemovalReceiptInjected(path, RemovalReceipt{}, inj); !errors.Is(err, errBoomPR8) {
				t.Fatalf("overwriteRemovalReceiptInjected(%s failure) = %v, want errBoomPR8", step, err)
			}
			matches, globErr := filepath.Glob(filepath.Join(dir, ".prune-receipt-*"))
			if globErr != nil {
				t.Fatal(globErr)
			}
			if len(matches) != 0 {
			...
			if _, statErr := os.Stat(path); !os.IsNotExist(statErr) {
				t.Fatalf("failed write published a visible receipt: %v", statErr)
			}
		})
	}
}

// TestOverwriteRemovalReceiptInjectedPublishesAt0600 is review-763's
// chmod-preset-Hook lesson applied to this site: os.CreateTemp already
// creates its temp file at 0600, the same as this site's own final chmod
// target, so a plain end-to-end 0600 assertion cannot tell a real chmod
// call from a deleted one.
func TestOverwriteRemovalReceiptInjectedPublishesAt0600(t *testing.T) {
	t.Parallel()
	dir := t.TempDir()
	path := filepath.Join(dir, "receipt.json")
	hookRan := false
	inj := &filewrite.Injector{Step: filewrite.StepChmod, Hook: func() {
		hookRan = true
		matches, err := filepath.Glob(filepath.Join(dir, ".prune-receipt-*"))
	...
	if err := overwriteRemovalReceiptInjected(path, RemovalReceipt{}, inj); err != nil {
		t.Fatal(err)
	}
	if !hookRan {

...
func TestSetPeersUpstreamInjectedHonoursInjectedFailures(t *testing.T) {
	t.Parallel()
	for _, step := range []filewrite.Step{
		...
		t.Run(string(step), func(t *testing.T) {
			...
			err := setPeersUpstreamInjected(path, "https://hub.example.com", "/abs/token", inj)
			if !errors.Is(err, errBoomPR8) {
				t.Fatalf("setPeersUpstreamInjected(%s failure) = %v, want errBoomPR8", step, err)
			}
...
// TestSetPeersUpstreamInjectedPublishesAt0600 is review-763's
...
func TestSetPeersUpstreamInjectedPublishesAt0600(t *testing.T) {
	t.Parallel()
	dir := t.TempDir()
	path := filepath.Join(dir, "config.yaml")
	hookRan := false
	...
	if err := setPeersUpstreamInjected(path, "https://hub.example.com", "/abs/token", inj); err != nil {
		t.Fatal(err)
	}
	if !hookRan {
...
func TestSetPeersUpstreamInjectedRejectsAnInvalidURL(t *testing.T) {
	t.Parallel()
	dir := t.TempDir()
	path := filepath.Join(dir, "config.yaml")
	if err := setPeersUpstreamInjected(path, "not-a-url", "/abs/token", nil); err == nil {
		t.Fatal("setPeersUpstreamInjected(invalid url) = nil, want an error")
	}
	matches, err := filepath.Glob(filepath.Join(dir, ".wb-config-*"))
	if err != nil {
		t.Fatal(err)
	...
	if len(matches) != 0 {
		t.Fatalf("invalid url still staged a temp file: %v", matches)
	}
}

...
func TestTailCovUnitsClassifiesEveryToolShape(t *testing.T) {
	cases := []struct {
		...
		{"golangci-lint", []string{"golangci-lint", "run"}, 3, 1},
		{"staticcheck", []string{"staticcheck", "./..."}, 3, 1},
		{"pytest", []string{"pytest", "-q"}, 3, 1},
		{"vitest", []string{"vitest", "run"}, 3, 1},
		{"jest", []string{"jest", "--ci"}, 3, 1},
		{"mocha", []string{"mocha"}, 3, 1},
		...
		{"nx lint", []string{"nx", "lint"}, 3, 1},
		{"nx e2e", []string{"nx", "e2e"}, 3, 1},
		...
		{"bun test", []string{"bun", "run", "test"}, 3, 1},
		{"npx lint", []string{"npx", "lint"}, 3, 1},
